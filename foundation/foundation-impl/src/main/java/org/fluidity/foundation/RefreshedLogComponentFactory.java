 ...
 * @author Tibor Varga
 */
@Component(api = Log.Refreshed.class)
@Component.Context(typed = true)
final class RefreshedLogComponentFactory implements CustomComponentFactory {

    private final Deferred.Reference<Long> period;

    public RefreshedLogComponentFactory(final Configuration<Settings> configuration) {
    ...
    static final class RefreshedLogImpl<T> implements Log.Refreshed<T> {
    ...
    private interface Settings {
         ...
         */
        @Configuration.Property(key = Log.Refreshed.LOG_LEVEL_CHECK_PERIOD)
        long period();
    }
}
